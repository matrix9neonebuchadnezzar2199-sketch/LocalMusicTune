...
**PHASE 4-4 — UIとの接続（2B）** ✅ 実装済（v0.4.5 で品質ガード追加）
4-3の生成関数を Gradio UI の「生成」ボタン・プロンプト欄・スライダー・`prompt_builder` と接続。Gradio 6 ジェネレータ方式で非ブロッキング。CLI と同一の `MusicGenerator.generate` パス。
検証：UI から **30秒×30ステップ**（bfloat16）または float32 で生成・再生。⚠️ bfloat16 + 30s×60step は無音/ノイズ（2026-06-07 実機再現済み）。
