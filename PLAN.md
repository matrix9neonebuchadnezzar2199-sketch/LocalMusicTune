...
実装は `app/models/registry.py`。repo_id は [ACE-Step model zoo](https://github.com/ace-step/ACE-Step-1.5#-model-zoo) 準拠。

### PHASE 3・4 の検証方針

- **PHASE 3**: 標準 (2B) と XL (4B base) の DL・保管済み表示。標準2Bは容量が小さく DL 検証が速いため最低ライン。UI の既定 DL ターゲットは XL。
- **PHASE 4**: 開発環境（20 GB）では **XL を主軸**に Radeon/ROCm で本番推論を検証。フォールバックとして標準 (2B) も確認。VRAM 警告 UI は継続。
