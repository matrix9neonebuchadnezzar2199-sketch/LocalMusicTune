...
**PHASE 4-4 — UIとの接続（2B）**
4-3の生成関数を、Gradio UIの「生成」ボタン・プロンプト欄・スライダー（長さ・ステップ数）・`prompt_builder` と接続する。生成後、UIの出力プレイヤーで再生できるようにする。進捗バーはこの時点では「生成中…」の不定表示でよい(実進捗の連動は後)。
検証：UI上でプロンプトと長さを指定して生成ボタンを押すと、2Bで曲が生成され、プレイヤーで再生できる。

**PHASE 4-5 — 進捗バーの実連動**
ACE-Stepの拡散ステップ進行をコールバックで取得し、UIの進捗バーへ実際の進捗（ステップ n/N、パーセント）を反映する。
...
これをPLAN.mdに追記する形でCursorにコミットさせれば、準備完了です。次は、最初のサブステップである PHASE 4-1（環境疎通）の具体的な実装指示文（Cursorに渡すプロンプト）を一緒に作りましょうか。